      ...
      {MAIN_SIDEBAR.map((item, id) => {
        return typeof item === "string" ? (
          ...
          <Link
            ...
            href={`${item.href}`}
            className={cn(
              "block border-b-4 border-r-4 border-border p-4 pl-7 text-lg font-base text-foreground/90 hover:bg-accent hover:text-foreground",
              item.href === pathname &&
                "bg-main text-main-foreground hover:bg-main hover:text-main-foreground",
            )}
          >
            {item.text}
          </Link>
